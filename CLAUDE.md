...
- 每 tick Phase 3 发送 `S2CCraftHighlightData` 给已启用高亮的玩家
- `getOutputInfo()` 按 patterns 顺序收集最多 3 个匹配输出（`isCpuCraftingOutput` 或 `cs.isRequesting`），形成处理队列
- `COOLDOWN_MS = 1000` — 供应器空闲后高亮保留时间
- `TrackerEntry.outputs` — 当前输出物品列表（`List<OutputItem>`），`null` 表示无输出
- Phase 3 中无输出的条目跳过发送（除非 stuck），框选立即消失

### 网络 (`network/payloads/S2CCraftHighlightData.java`)
- `HighlightEntry(BlockPos pos, int statusOrdinal, List<OutputItem> outputs)`
- `OutputItem(ResourceLocation itemId, int outputType)` — outputType: 0=物品, 1=流体, 3=化学品

### 客户端渲染 (`client/render/CraftHighlightRenderer.java`)
- `RenderLevelStageEvent.AFTER_PARTICLES` 阶段渲染
- box fill（外发光+填充）+ box outline（边框）+ sprite billboard（物品图标队列）
- 最多 3 个 sprite 并排显示（间距 0.25，大小 0.28，整体不超过 1 格宽度）
...
- 网络定位器 (`network_locator`) 的贴图借用 AE2 的 `ae2:item/network_tool`，版权归 Applied Energistics 2 所有。
